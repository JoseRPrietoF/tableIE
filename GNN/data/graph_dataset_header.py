    ...
    def _process(self):
        ...
        for raw_path in self.flist:
            ...
            if not os.path.exists(raw_path):
                print(f"File {fname} doesnt exist! Check it please")
                continue
            f = open(raw_path, "rb")
            ...
            info_edges = data_load.get("edge_features", data_load.get("edges_features"))
            ...
            info_edges = np.ones((len(info_edges), 1)) #TODO DELETE!!!
            # print(nodes.shape)
            # print(info_edges.shape)
            # exit()
            ...
            # print(labels_tensor)
            # print(f"{fname} labels sum: {labels_tensor.sum()} from {len(labels_tensor)}")

...
def evaluate_graph_IoU(file_list, results, min_w = 0.5, th = 0.8, type_="COL", conjugate=True, all_edges=False, pruned=False, ORACLE = False):
    ...
    for raw_path in file_list:  
        ...
        if not os.path.exists(raw_path):
            print(f"File {raw_path} doesnt exist! Check it please")
            continue
        f = open(raw_path, "rb")
        ...
        edge_features = data_load.get("edge_features", data_load.get("edges_features"))
        gts_span = data_load.get("gts_span", None)
        min_nodes = 100
        if type_ == "span":
